...
**Screen for me** — a desktop screenshot app built with **Tauri v2** (Rust backend, TypeScript + Vite frontend, Konva canvas editor). v1 scope: area/window/fullscreen capture, full-display screen recording with optional microphone narration (macOS 15+), a bottom-left quick-access overlay, an in-app annotation editor, drag-out, copy/save. Targets: macOS (primary) and Linux; Windows later.
...
The app is a **menu-bar utility** (macOS ActivationPolicy::Accessory — no Dock icon). Everything starts from the tray menu or global shortcuts (Cmd/Ctrl+Shift+7/8/9 = area/window/fullscreen, Cmd/Ctrl+Shift+0 = start/stop recording, defined in `src-tauri/src/shortcuts.rs`).
...
- `src-tauri/src/history.rs` — capture files named `capture-<unix-ms>.{png,mp4}`; `CaptureEntry` carries a `kind` (`image`/`video`) and, for video, a `poster` sidecar path (`capture-<ms>.mp4.png` from `qlmanage`); ids are bare file names (path traversal rejected in `resolve`). `new_capture_path` takes the extension; `prune` keeps 50 entries total and at most 10 videos, deleting a video's poster with it.
- **Screen recording** (macOS 15+ only, `src-tauri/src/capture/recording.rs`): native ScreenCaptureKit via `objc2`, using `SCRecordingOutput` so the framework writes the `.mp4` itself (H.264, 30 fps, native Retina pixels, cursor shown; mic mixed in via `captureMicrophone`, default device, no system audio). `recording::supported()` gates on `objc2::available!(macos = 15.0)`; older systems get the `recorder.unsupported_*` dialog. `toggle_recording` (tray/shortcut) opens the transient `recorder` HUD (mic toggle + Record, remembers the last mic choice in `recorder_prefs.json`, a separate store like `editor_prefs.json`); the HUD's Record calls `start_recording`, which mirrors `run_scrolling_capture` (CAS guard on `record_running`, hides the overlay, `spawn_blocking`), then shrinks the HUD to the recording pill and spawns a watcher (`Recorder::wait_finished`) that writes the poster, publishes to history, destroys the HUD, and rebuilds the tray. Stop paths (pill Stop, Esc, the shortcut again, the tray item) all call `Recorder::request_stop`; the macOS menu-bar recording indicator arrives as `stream:didStopWithError`. **Gotchas**: `NSMicrophoneUsageDescription` (in `src-tauri/Info.plist`) is mandatory or TCC kills the process on mic access; `content_protected` does *not* hide windows from ScreenCaptureKit on macOS 15+, so the app excludes its own process in the `SCContentFilter`; the `Retained` stream/output/delegate must stay alive for the whole recording or callbacks stop silently; the `stopCapture` completion — not `recordingOutputDidFinishRecording` — is the finalised signal. The pure helpers (`pick_display`, `pill_position`, `validate_recording`) are unit-tested; the ObjC lifecycle is not.
